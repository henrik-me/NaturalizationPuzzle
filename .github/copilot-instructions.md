...
- **Default to background even for "mandatory" reviews.** Sync sub-agent calls block the orchestrator from receiving user input until the agent returns. For the GPT-5.6 Sol plan/diff reviews and other long-ish reviews, launch in **background**, end the turn, and resume from the completion notification. Only use `mode: "sync"` when (a) the agent is expected to return in well under 10 seconds, or (b) the orchestrator literally has nothing meaningful to do or say to the user until the result arrives. **A pending review is not a license to ignore the user.**
...
- **Code-review / rubber-duck reviews (plan reviews and final-diff reviews):** use `model: "gpt-5.6-sol"`. This is the only model used for review work, including the local pre-push GPT-5.6 Sol review and the GPT-5.6 Sol plan review.
- **Code, test, and implementation-plan generation** (`general-purpose`, `task`, and any sub-agent that writes/edits source code, writes/runs tests, or produces an implementation plan for the orchestrator to execute): use `model: "claude-opus-4.8"` (Claude Opus 4.8). This is the strongest implementation model and holds the full repository context the sub-agent needs for non-trivial implementation and test work. Note: this covers *generating* implementation plans; *reviewing* a plan is still a code-review task and uses `gpt-5.6-sol` per the bullet above.
- **Explore agents** doing read-only investigation may use the agent type's default fast model. If a specific exploration genuinely benefits from deeper reasoning, override to `claude-opus-4.8`.
- **Monitoring / polling sub-agents** (e.g., a `task` agent whose only job is to repeatedly poll `gh pr view` or `gh run view` until a CI run finishes, or to tail a log until a marker appears): use a fast/cheap model — `claude-haiku-4.5` is the recommended default. There is no reasoning-heavy work for a stronger model to do, and Opus 4.8 would be wasteful for a 25-minute sleep loop. The reasoning that decides *what to do with the result* happens in the orchestrator after the polling agent returns.
- **Fallback when `claude-opus-4.8` is unavailable** (e.g., the sub-agent invocation errors with model-access denied, or an external contributor lacks access to the model): substitute the strongest available high-reasoning Claude Opus model with the largest context window the environment offers (e.g., `claude-opus-4.7-1m-internal`, `claude-opus-4.7`, then `claude-opus-4.6`). State the substitution to the user in the same response that launches the sub-agent. If no Claude Opus model is available, ask the user how to proceed rather than silently falling back to a much weaker model.
...
##### Review sub-agents (`code-review`, `rubber-duck`, GPT-5.6 Sol plan/diff reviews)
...
- **Every change must receive a local GPT-5.6 Sol review of the final diff before it is pushed or opened as a PR. No exceptions** — this applies to features, fixes, refactors, dependency bumps, infrastructure, workflows, scripts, **and documentation-only changes**.
- Perform the review by invoking the code-review sub-agent (e.g. `rubber-duck`) with `model: "gpt-5.6-sol"`. **The review MUST run as a sub-agent — never review the diff inline in the orchestrator.** Inline review consumes orchestrator context for material the orchestrator only needs the *conclusions* of. The sub-agent counts as a local review.
...
- The review sub-agent prompt MUST also include the **Review Depth Checklist** below. Without an explicit checklist, GPT-5.6 Sol reviews tend to spot-check obvious diffs and miss the categorical issues (brittle tests, stale comments, ordering dependencies, factual content claims) that Copilot then catches across multiple rounds. Prescribing the checklist up front lets the local review surface those findings before the push.
- For **non-trivial** changes (multi-file, architectural, security-sensitive, or dependency/infra), also do a **plan review** with GPT-5.6 Sol *before* implementing. Plan review may be skipped only for trivial changes (single small edit, typo fix, renaming).
...
- **Every non-docs change must pass full local verification before it is pushed or opened as a PR, and again before every subsequent push to the PR branch** (e.g., commits that address Copilot or GPT-5.6 Sol review feedback). This is a mandatory pre-push gate, peer to the GPT-5.6 Sol review above.
...
- Verification runs on the change's affected sides; at minimum:
  - **Frontend changes** (`src/client/**`): `npm run lint && npm test -- --run && npm run build && npm run size` from `src/client/`.
  ...
  - **Container E2E (production-image Playwright)**: changes that touch `Dockerfile`, `.dockerignore`, `src/api/NaturalizationPuzzle.Api.csproj` (especially `<EmbeddedResource>` includes), `content/stories/**`, embedded resource source paths, or the static-asset pipeline (`src/client/vite.config.ts`, build output → `wwwroot/`) must additionally run the container E2E reproducer: `.\scripts\container-e2e.ps1` (Windows) or `./scripts/container-e2e.sh` (Linux/macOS/WSL). The dev-stack E2E **cannot** catch Docker-context-only bugs (the PR #81 silent-empty-stories incident is the canonical example). The container E2E may be skipped only when the change is image-output-neutral — i.e., it provably cannot alter what ships inside the built image (e.g., a test-only change under `tests/`, or a docs-only edit). If you cannot prove output-neutrality, run it (~5 min added, requires Docker installed).
...
The same rule applies to **planning scaffolding when a planning item ships**: when flipping a CONTEXT.md item from "planned" to "shipped" (✅), the same commit MUST remove every downstream paragraph that assumed the old status — stream/phase breakdowns, codebase findings, open decisions, effort & critical-path estimates, "next steps" bullets. A `✅ complete` header above a still-live stream breakdown is a contradiction the reader has to resolve, and GPT-5.6 Sol reviews correctly flag it as major. Item #20's shipping commit (`5449ae5`) is the canonical example — the first edit only updated the header line and left 40+ lines of planning scaffolding intact; the second edit removed them atomically.

#### Audit Whole-File for Class of Issue

When a reviewer (Copilot or GPT-5.6 Sol) flags one occurrence of a class of issue — hardcoded count, missing citation, stale comment, magic number, etc. — **audit the entire affected file (and sibling files of the same kind) for every other instance of that class in one pass**. The motivation is **fix completeness**, not minimizing review rounds: a partial fix that addresses only the flagged occurrence leaves siblings in an obviously inconsistent state, which is itself a defect. Bias toward over-fixing within the class; under-fixing is the more expensive mistake because it ships a known-incomplete change.
...
3. **`external-review-policy` App check is `SUCCESS` on the current head.** Production branch protection (ruleset 15368163) sets `required_approving_review_count` to 0 with an empty bypass list, so the enforcement gate is the App-bound `external-review-policy` required check — **not** a native `reviewDecision == APPROVED`. This check is **independent of the CI checks**: the CI contexts (Build & Test, Analyze csharp/javascript-typescript, E2E Playwright) and `external-review-policy` are separate required gates that all must pass, and the App check does **not** wait for or depend on CI. The App writes `external-review-policy` to success based on trusted authorship or a current-head owner approval, but that condition is **necessary, not sufficient**: success is produced only for a **unique protected head after a successful fail-closed reconciliation**. Duplicate-head detection (App-authored PR #177, `6e15df2`) deliberately blocks success when two open protected PRs share one head SHA, and any reconciliation error also fails closed. So for owner-authored PRs (including PRs the orchestrator opens on the owner's behalf via `gh pr create`) the check succeeds on trusted authorship alone once its head is unique and reconciliation succeeds — no human `APPROVED` review is required and self-approval is neither needed nor possible; for external- or bot-authored PRs it turns `SUCCESS` only after `henrik-me` submits an `APPROVED` review of the authoritative current head, again subject to the unique-head fail-closed condition. **Important:** Copilot's PR reviews are always submitted as `COMMENTED`, never `APPROVED`, and never satisfy the external-review gate. A clean Copilot loop is necessary but not sufficient. Verify the check via `statusCheckRollup`; `reviewDecision` may legitimately be empty on owner PRs and is not the gate.
...
Any change that is **not docs-only** must additionally pass an iterative GitHub Copilot review on the pull request itself. This is in addition to (not a replacement for) the local GPT-5.6 Sol review above. "Docs-only" here means the change touches only paths covered by the CI/CD workflow's `paths-ignore` list (the enumerated top-level docs — `README.md`, `CONTEXT.md`, `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md`, `LastSession.md`, `src/client/README.md`, `LICENSE`, `.gitignore`, `.editorconfig` — plus copilot/contributor instructions and PR/issue templates). **Story Mode catalog files at `content/stories/*.md` are product code, NOT docs** — they ship as `<EmbeddedResource>` in the API DLL, so a content-only edit must run CI, smoke test, and deploy. Treat such PRs as non-docs.

- **PR required.** Never push non-docs changes directly to `main`. **`gh pr merge --admin`, ruleset edits, bypass actors, and every other admin/protection bypass are unconditionally prohibited** — there is no exception, including PRs the orchestrator opens on the user's behalf. Production branch protection (ruleset 15368163) has an empty bypass list and `required_approving_review_count` 0, so an owner-authored PR (including one authored on the user's behalf) does **not** need a native `APPROVED` review: the dedicated App writes the `external-review-policy` check to success on trusted authorship — necessary but not sufficient, since success is produced only for a unique protected head after successful fail-closed reconciliation (duplicate-head detection and any reconciliation error deliberately block success) — independently of CI. That App check and the CI contexts are separate required gates — all must pass, and the App check does not wait on CI — so the PR merges normally once every gate is green. Self-approval is therefore never required and never attempted. If a merge is blocked, resolve the actual failing gate (CI, unresolved threads, the App-bound check, mergeability) or escalate to the user — never force the merge with `--admin` or by weakening settings.
...
- The local pre-push GPT-5.6 Sol review is still required for every commit pushed to the PR branch — including commits that address Copilot's feedback.
...
**Plan review:** dependency/infra changes are classed as non-trivial under the **Code Review** section, so a GPT-5.6 Sol plan review applies. In practice, for a routine patch-level Dependabot PR (no breaking changes, narrow blast radius), the validation checklist below is itself the plan; for minor or major bumps run a separate plan review before starting validation.
...
5. **Lint, test, build** on the affected side:
   - Frontend (run from `src/client/`): `npm run lint && npm test -- --run && npm run build && npm run size`
...
6. **Run the GPT-5.6 Sol `code-review` sub-agent** on the final diff (mandatory per Code Review section). The reviewer can read from the same worktree.
7. **Re-run the validation checklist (steps 2–6) after every fix-up commit** pushed to the PR branch in response to Copilot or GPT-5.6 Sol review feedback. The pre-push verification gate already covers build + unit tests + e2e for the new commit; this step ensures the diff scope, resolved versions, and final-diff GPT-5.6 Sol review reflect the latest PR head before merge.
...
- **Keep the two scripts behaviorally equivalent.** When one side changes (e.g., a new health-check field, an extra container flag), the sibling must change in the same commit. GPT-5.6 Sol / Copilot reviews routinely flag drift between the `.ps1` and `.sh` variants.

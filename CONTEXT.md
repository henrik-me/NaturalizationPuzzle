...
- `.github/workflows/external-review-policy.yml` + `external-review-policy-signal.yml` — repository-local App-owned review policy with a secretless review wake-up and authoritative default-branch API refetch; rolled out with a no-bypass App-authored bootstrap, target Actions secrets, and canary proofs, and the production ruleset migration is applied (see `docs/`). Duplicate-head hardening landed via App-authored PR #177 (main commit `6e15df2`) and duplicate protected heads are failed closed in code; the workflow writes only the App-bound check and creates no App approval/review; steady-state token permission is Checks write, Metadata read, Pull requests read (Pull requests write only temporarily for App-authored publication). Owner PRs merge on CI + the App-bound `external-review-policy` check with zero native approvals; external/bot/Dependabot PRs require the owner's current-head APPROVED review via that check during successful reconciliation of a unique head (subject to the documented residual timing risks in `docs/external-review-policy-threat-model.md`). Only remaining work: retire the disposable `policy-canary` branch and its temporary ruleset (this cleanup drops `policy-canary` from the policy workflow and the CI/CodeQL PR filters)
...
- Program.cs: DI registration, CORS, **response compression (Brotli + Gzip, `Fastest`, `application/json` + `application/problem+json`, `EnableForHttps = true`)**, static file serving, SPA fallback, auto-create DB on startup, HTTPS redirect (dev only), Application Insights via OpenTelemetry (conditional)
...
- ~~`GET /api/v1/representatives` (no subpath) silently returns the SPA `index.html` because the `MapGroup` defines only `/vacant`, `/{id}` (PUT), and `/reset` — request falls through to `MapFallbackToFile`. Latent (no client consumer) but surprising for anyone exploring the API. Tracked in **#72**.~~ ✅ Resolved in PR #99: added `MapGet("/")` named `GetRepresentatives` with optional `?stateId=` filter, returning a JSON array (`Results.Ok`). New regression test in `RepresentativesEndpointTests.cs` asserts JSON 200 + non-empty array for the bare URL.
...
- ~~`src/client/package.json` has no `engines.node` field, so `npm install` doesn't enforce Node 20.19+ even though `@vitejs/plugin-react@5.x` requires it. README also says "Node.js 20+" which under-specifies the patch version. Tracked in **#70**.~~ ✅ Resolved in PR #94: `engines.node` is now `^22.19.19 || ^24.0.0` and `src/client/.npmrc` enables `engine-strict=true`, so mismatched Node versions fail `npm install` outright. The floor tracks the `@types/node` pin to keep the types from claiming APIs the runtime lacks. README prerequisites updated to match.
...
- **Dockerfile build context vs `EmbeddedResource` Include paths.** The Dockerfile builds the API from `src/api/` only, but `src/api/NaturalizationPuzzle.Api.csproj` declares `<EmbeddedResource Include="..\..\content\stories\*.md">` and `*.sources.json` — paths that escape `src/api/`. PR #81 added `COPY content/ content/` to the Docker build context after observing that prod returned `[]` from `/api/v1/stories` despite the same endpoint returning the full catalog from a local `dotnet run`. MSBuild resolves `EmbeddedResource Include=` paths relative to the **csproj file's directory**, so the Include literally targets `<dir-of-csproj>/../../content/`. In a normal local build the csproj sits at `<repo>/src/api/`, the relative path resolves to `<repo>/content/`, and the Includes match. Inside the Docker build context the csproj is COPYed to `/app/src/api/` but `/app/content/` doesn't exist unless explicitly added — without `COPY content/ content/` the Includes silently match zero files because the **build-context contents** lack the target directory, `dotnet publish` finishes without warnings, and the image ships without any embedded story resources. **This bug almost certainly affected PR #74 (the v1 pilot) too** — Story Mode pilot likely never actually loaded in production until PR #81 fixed the Dockerfile. **Ongoing rule**: any future `<EmbeddedResource Include="...">` that escapes `src/api/` MUST be paired with an explicit `COPY` of that path into the Docker build context (plus a `.dockerignore` exception if the file extension is globally excluded) AND a smoke-test assertion that the endpoint serving the resource returns a non-empty payload. PR #81 added the `/api/v1/stories` assertion to the Image Smoke Test step in `.github/workflows/ci-cd.yml` so this exact regression is caught at the smoke step before the deploy ever runs.
...
- **Fork PR workflow approval ("require approval for all outside collaborators")** — **not available for personal-account repositories.** This UI toggle and the matching `actions/permissions/fork-pr-workflows` API endpoint exist only for organization-owned repos and enterprise accounts. Personal repos get a fixed GitHub default: workflow runs from **first-time contributors** require manual approval; subsequent runs from the same contributor are auto-approved. To get the explicit "approve all outside collaborator runs" toggle, the repo would need to be transferred to a (free) GitHub Organization. Residual risk on this personal repo is mitigated by:
  ...
  - Branch ruleset on `main` requires PR + the App-bound `external-review-policy` check + status checks (a fork PR needs the owner's current-head approval to land code during successful reconciliation of a unique head, subject to the documented residual timing risks in `docs/external-review-policy-threat-model.md`; duplicate protected heads are failed closed in code via PR #177/`6e15df2`; there is no admin/RepositoryRole bypass).
...
6. ~~**Resolve outstanding Dependabot version-bump PRs** (Phase 6)~~ ✅ complete — 3 merged PRs on 2026-04-21 closed 13 Dependabot PRs and hardened `dependabot.yml`:
   ...
   - **Blocked bumps**: Vite 8 (blocked by `@tailwindcss/vite` peer range ^5‖^6‖^7) and Node 25-alpine (Node major bumps are manually managed per `.github/dependabot.yml`, kept in sync with `src/client/package.json` `engines.node`). Both ignored in `.github/dependabot.yml` (commit `9784380`) — #12 and #22 auto-closed.
...
21. **Post-deploy Playwright E2E against production** (follow-up to #20 — **depends on #20 landing first**).
    ...
    **Scope:**
    - Smoke-flavored subset of the Playwright suite against `https://np.metzger.dk` (and the default Container Apps FQDN) after `deploy-apply` succeeds.
    - Run the main user flows: state selection, study, quiz, stories index + detail, offline. **Skip axe-core deep checks** (they are container-equivalent — already covered by #20's container-e2e job).
    - Cold-start tolerance: first request may take ~22 s on a scale-to-zero replica; bump per-test timeouts accordingly or run a `/api/health` warm-up step before Playwright starts.
    - Failure handling: the job's failure blocks the next deploy via job dependency, AND **should trigger a rollback** to the previous Container App revision (`az containerapp revision activate --revision <previous>` — verify the previous revision is still in `Provisioned` state before swapping; if not, fail loudly and alert).
    - New CI job: `post-deploy-e2e` with `needs: deploy-apply`, runs against the deployed `${app-url}` (or custom domain).
    - **Open question to lock when starting this work**: rollback automation vs alert-only on failure. Auto-rollback is safer for the user but requires extra plumbing (lookup previous revision, swap traffic). Alert-only is simpler and matches the current "human-gated deploy" posture. Recommendation: start with alert-only, add auto-rollback if a real prod regression motivates it.
...
The most recent things shipped, all live in production at `https://np.metzger.dk` (or pending the production approval gate):

0. **Answer-checker precision fixes** (issue #87, three sequential gated PRs: **#116 `4f40841`**, **#117 `dd17673`**, **#118 `a6ad675`**) — the quiz/story typed-answer checker no longer rejects valid answers. Delivered as three classes, each shipped with heavy false-positive sentinel tests: **(A) numeric digit↔word equivalence** (#116 — typing `4`/`9`/`5` is accepted for `Four (4) years`/`Nine (9)`/`Five (5)`; whole-token numeric matching so `1` never matches `100`; ordinals, year word-forms, required-numeric-token gating); **(B) typo tolerance** (#117 — bounded Levenshtein ≤1 for ≥6-char non-digit tokens, with a plural guard and a lone-fuzzy guard to kill FPs like `state`/`states` and "Presidents Day"≈President); **(C) whole-answer synonyms** (#118 — a tiny exact-only curated map, e.g. `USA`/`America`→"The United States", `E pluribus unum`→"Out of many, one", matched **only** against the stripped canonical candidate so it adds zero partial-match surface). All logic lives in the single pure module `src/client/src/utils/answerChecker.ts` (consumed by `QuizPage.tsx` + `StoryPage.tsx` via `checkAnswer`); 56+ tests in the co-located `answerChecker.test.ts`. Reviewed via GPT-5.5 plan/diff passes + Copilot rounds; all admin-squash-merged. Deployed and verified live via CI/CD run 26936756139 (`/api/health` → `{"status":"healthy","database":true,"questionCount":128}`). Issue #87 closed.
1. **API performance baseline** (PR #96) — three server-side fixes after a full three-axis perf audit (frontend, backend, infra): (a) `UseResponseCompression` with Brotli + Gzip at `CompressionLevel.Fastest`, configured for `application/json` and `application/problem+json`, `EnableForHttps = true` (payloads are public read-only civics data so CRIME/BREACH does not apply); (b) `AsNoTracking()` applied to every read-only EF Core query across `QuestionService`, `QuizService`, and `RepresentativeService` read paths (mutation paths intentionally untouched); (c) `StateService` collapsed from "load all states + load all reps + match in memory" to a single EF projection with a correlated subquery, refactored through a shared `ProjectToDto(IQueryable<UsState>)` helper so `GetAllStatesAsync` and `GetStateByIdAsync` compose the same expression tree. New `StateServiceTests` (SQLite in-memory, including a synthetic out-of-order seed for the ordering contract). Multi-round Copilot review before clean. **Known gap (tracked as issue #97):** full perf measurement infrastructure is landing in layers. **Layer 3 (size-limit bundle guard) shipped in PR #103** — `size-limit` runs in CI after `npm run build` and enforces a brotli-compressed budget on `dist/assets/index-*.{js,css}`; current baseline is well under both budgets. **Layer 1a (web-vitals client scaffold) shipped in PR #104** — Core Web Vitals (`LCP`, `INP`, `CLS`, `FCP`, `TTFB`) log to the browser console at runtime via `src/client/src/perf/webVitals.ts`. **Layer 1b (Lighthouse CI synthetic gate) shipped in PR #105** — `npx lhci autorun` runs three Lighthouse passes against `vite preview` and asserts LCP ≤ 2500 ms (`error`), CLS ≤ 0.1 (`error`), accessibility ≥ 0.9 (`error`), and performance ≥ 0.85 (`warn`, will tighten after baseline); reports upload as the `lighthouse-reports` artifact. The synthetic run intentionally targets `vite preview` without a backing API — the client's warm-up calls resolve to the SPA's `index.html`, which is bounded (LCP unaffected, budgets pass) but means the synthetic numbers are slightly more conservative than production; tracked as follow-up issue [#106](https://github.com/henrik-me/NaturalizationPuzzle/issues/106). A second follow-up [#107](https://github.com/henrik-me/NaturalizationPuzzle/issues/107) tracks de-duplicating the client build between the `build-and-test` and `lighthouse` jobs. **Layer 2 (k6 API benchmark, advisory) shipped in PR #109** — `tests/perf/api-bench.js` runs a k6 smoke + load profile against a locally-built API (`dotnet run -c Release --no-launch-profile` with `ASPNETCORE_ENVIRONMENT=Production` to bypass `launchSettings.json` and disable the dev-only HTTPS redirect that would otherwise skew measurements), hitting the seven hot read endpoints: `/api/v1/questions`, `/api/v1/questions/6520?stateId={id}`, `/api/v1/questions?stateId={id}`, `/api/v1/states`, `/api/v1/states/{id}`, `/api/v1/stories`, and `/api/v1/stories/{slug}?stateId={id}` (where the story slug is resolved dynamically in `setup()` from the live stories index so we never hardcode a slug). `tests/perf/summarize.mjs` consumes the k6 JSON output and emits a per-endpoint p50/p95/p99 Markdown table (no thresholds gating yet — advisory only, baselined over the next ~10 runs). The new `API Benchmark (k6, advisory)` job in `.github/workflows/ci-cd.yml` uses `grafana/setup-k6-action@v1` pinned to k6 `2.0.0`, redirects the Markdown to `tests/perf/k6-summary.md`, cats it inline for log visibility, and uploads `k6-results.json` + `k6-summary.md` + `api.log` as the `api-bench-results` artifact. Shared `ENDPOINT_TAGS` between `api-bench.js` and `summarize.mjs` live in `tests/perf/endpoint-tags.mjs`. **Layer 1.5** (production telemetry sink — App Insights JS SDK or custom `/api/v1/perf/web-vitals` endpoint, tracked in [issue #108](https://github.com/henrik-me/NaturalizationPuzzle/issues/108)) is the remaining planned follow-up.
2. **Story comprehension typed-input quiz mode + scored history** (PR #86) — second comprehension-quiz mode on every story page (two-CTA `Continue with Study` / `Continue with Quiz` chooser, no default), scored attempts persisted in a new `storyQuizHistory` array, History page gains a "Story Comprehension" block (Stats / Per-Story aggregation / chronological list with × delete + 7s inline Undo). Six pre-push verify rounds + six GPT-5.5 final-diff review rounds + three Copilot PR review rounds caught five real race conditions (idempotent restore on double-Undo, idempotent submit on double-click, idempotent next/study/results advance handlers — all in StoryPage; the latter three were audit-whole-file extensions of the original Copilot finding). See [issue #85](https://github.com/henrik-me/NaturalizationPuzzle/issues/85) for the full plan and decision log; Phase 19 in [Next Steps](#next-steps) for the shipped surface area.
3. **Story Mode full-catalog expansion** (PR #77) — every USCIS subcategory now has at least one dedicated story, with multi-membership where a question fits multiple topics, so every one of the 128 civics questions is claimed by at least one story. The previous Story Mode milestone was **v1 (pilot)** (merged via PR #74). See Phase 14 (pilot) and Phase 15 (catalog) in [Next Steps](#next-steps) for the full surface area, and the [Story Mode — Authoring Guide](#story-mode--authoring-guide) section for how to add a story.

Active state at the time of writing this guide:

- `main` advances through reviewed squash/normal merges; at the time of writing its tip is `6e15df2` (App-authored PR #177, duplicate-head hardening). Production is up-to-date or pending the `production` GitHub-environment approval gate for the most recent deploy-relevant tip.
...
- 0 open Dependabot alerts on NaturalizationPuzzle (all severities) at the time of writing; the App-bound review policy is live on `main` with the ruleset migration applied and duplicate-head hardening complete (App-authored PR #177, `6e15df2`). Remaining work is retiring the disposable `policy-canary` branch and its temporary ruleset (#21943248).

### Where the work that's still on the table lives

| What | Where |
|---|---|
| All deferred / postponed work | [Next Steps](#next-steps) — items 9, 16, 17, 18, 20, 21 are open (items 14, 15, 19 are ✅ complete) |
| Existing tech debt | [Known Issues / Tech Debt](#known-issues--tech-debt), plus tracked GitHub issues (#73) |
...
npm run lint && npm test -- --run && npm run build && npm run size

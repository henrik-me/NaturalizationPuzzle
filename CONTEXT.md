...
- ~~`src/client/package.json` has no `engines.node` field, so `npm install` doesn't enforce Node 20.19+ even though `@vitejs/plugin-react@5.x` requires it. README also says "Node.js 20+" which under-specifies the patch version. Tracked in **#70**.~~ ✅ Resolved in PR #94: `engines.node` is now `^22.13.17 || ^24.0.0` and `src/client/.npmrc` enables `engine-strict=true`, so mismatched Node versions fail `npm install` outright. README prerequisites updated to match.
